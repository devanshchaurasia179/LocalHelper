...
import type { Partner } from '@/providers/AuthProvider';
...
export default function VerifyOtpScreen() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Navigate to the first incomplete onboarding step, or home if all done
      router.replace(resolveNextRoute(partner) as any);
    } catch (err: any) {
      const msg =
  ...
  /** Returns the route the partner should land on after login */
  function resolveNextRoute(partner: Partner): string {
    if (!partner.isProfile)  return '/onboarding/profile';
    if (!partner.isService)  return '/onboarding/service';
    if (!partner.isDocument) return '/onboarding/documents';
    return '/';
  }
            ...
            <View style={styles.otpRow}>
              {digits.map((d, i) => (
                <TextInput
                  ...
                  value={d}
                  onChangeText={(t) => {
                    // Handle paste via onChangeText when full string is received
                    if (t.length > 1) {
                      handlePaste(t);
                    } else {
