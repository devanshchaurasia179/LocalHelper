...
export function useChatRoom(conversationId: string): UseChatRoomResult {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        // connectChatSocket() never rejects — it always returns the socket
        // instance even if the initial connection is still in progress.
        ...
        // callerType in the event is the OTHER party's type (partner for customer app).
        const onUserPresence = ({
          ...
          if (cId !== conversationId) return;
          // Only update presence for the other party (partner), not ourselves
          if (presenceType === "partner") {
            setIsOtherOnline(isOnline);
          }
        };

        // ── messages_read ──────────────────────────────────────────────────
        // Fired when the other party (partner) reads our messages.
        const onMessagesRead = ({
          ...
          if (cId !== conversationId) return;
          // Mark all our (customer) sent messages as read
          if (readBy === "partner") {
            setMessages((prev) =>
              prev.map((m) =>
    ...
    // setup() returns a cleanup fn — store it
