...
const CALLKEEP_OPTIONS = {
  ...
  android: {
    ...
    cancelButton: 'Cancel',
    okButton: 'OK',
    // Self-managed ConnectionService is declared in AndroidManifest.xml
    additionalPermissions: [],
    // Foreground service so the call survives Doze / background.
    foregroundService: {
      channelId: 'com.localhelpers.customer.callkeep',
...
export async function setupCallKeep(): Promise<void> {
  ...
  try {
    ...
    if (Platform.OS === 'android') {
      // Required for the self-managed ConnectionService to place/receive calls.
...
function attachListeners(): void {
  ...
  RNCallKeep.addEventListener('answerCall', ({ callUUID }) => {
    ...
    // Bring the app/room to focus and let CallKeep know we're connecting.
  ...
  RNCallKeep.addEventListener('endCall', ({ callUUID }) => {
    ...
    // If the call was still ringing (never answered) this is a reject;
    // otherwise it's an end/hang-up. useCallManager decides based on its state.
    callBridge.emit('end', { callId, wasRinging: wasDisplayed });
  });

  // Mute toggled from the native UI (best-effort forward).
