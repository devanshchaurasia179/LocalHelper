 ...
 *   3. POST it to /api/customer/fcm-token so pushes can be delivered.
 ...
 *
 * Auth is cookie-based (customer_token httpOnly cookie) and sent automatically
...
async function sendTokenToServer(token: string): Promise<void> {
  await api.post("/customer/fcm-token", { token, platform: PLATFORM });
  _currentToken = token;
...
export async function registerPushToken(): Promise<void> {
  try {
    ...
    if (!token) {
      console.warn("[Push] getToken() returned empty — skipping");
      return;
    }
...
export async function unregisterPushToken(): Promise<void> {
  try {
    ...
    if (token) {
      await api.delete("/customer/fcm-token", {
